...
- **Web demo:** loads prefilled with this case's account history (from the
  ...
  history — widen the range to bring earlier context into view. New investigator
  notes are timestamped automatically when added (no dates to type), and the range
  auto-widens if a new note falls outside it. Gap flagging states the size of the
  gap directly, rounded to the nearest whole day (e.g. "Gap of 2 days") rather
  than a generic alert label or a decimal.
