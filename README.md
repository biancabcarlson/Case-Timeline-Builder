...
- **Web demo:** loads prefilled with this case's full account history (from the
  `simulated-account` fixture — account opening through the ATO incident window),
  builds the timeline automatically on page load, and renders it visually (numbered
  markers, one entry per row) with gap warnings attached inline to the relevant pair
  of events — no manual entry required to see a populated timeline, matching how the
  other tools in this series work. The notes stay fully editable — add, remove, or
  rewrite any line and rebuild. Lines that don't start with a recognizable date are
  reported by line number instead of silently dropped.
